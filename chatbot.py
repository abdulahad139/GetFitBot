...
class FitnessCoachChatbot:
    ...
    def setup_llm(self): # Initialize the Groq Llama 3 model
        self.llm = ChatGroq(groq_api_key=GROQ_API_KEY,model_name=MODEL_NAME,temperature=TEMPERATURE,max_tokens=MAX_TOKENS,streaming=False)
    
    def setup_memory(self): # Setup LangChain ConversationSummaryBufferMemory for conversation context
    ...
    def setup_conversation_chain(self): # Setup the conversation chain with custom prompt
        self.prompt_template = ChatPromptTemplate.from_messages([("system", SYSTEM_PROMPT),MessagesPlaceholder(variable_name="chat_history"),("human", "{input}")])
    
    def generate_response(self, user_input: str, user_profile: dict):
        # Checking if user profile is complete with at least one goal selected
        ...
        try:
            ...
            chain = LLMChain(llm=self.llm,prompt=self.prompt_template,verbose=False)
